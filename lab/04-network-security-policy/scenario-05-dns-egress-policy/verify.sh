...
set -euo pipefail
NS=sec-lab5
ALLOWED=$(kubectl -n "$NS" exec deploy/client -- curl -s -o /dev/null -m 5 -w '%{http_code}' https://raw.githubusercontent.com || true)
BLOCKED=$(kubectl -n "$NS" exec deploy/client -- curl -s -o /dev/null -m 5 -w '%{http_code}' https://example.com || true)
