...
Also enable Cilium's L2 announcements so the Gateway's `LoadBalancer`
Service gets a real, reachable address on your docker network (without it,
`status.addresses` never populates on bare kind — see the honest note
below).
...
own L2 announcements feature, enabled above). With L2 announcements on, live
traffic through the Gateway does work — but can be flaky for a minute or two
right after any Cilium/Gateway API config change while Envoy finishes
reconciling. If `verify.sh`'s live-traffic check fails right after setup,
wait ~30s and rerun it before assuming something is actually broken.
...
pointing at the same Gateway, so real traffic actually splits between both
versions without the client knowing.

## Definition of done
- [ ] `checkout-route` has exactly two `backendRefs`: `checkout-v1` with
      `weight: 90` and `checkout-v2` with `weight: 10`
- [ ] Requests through the Gateway succeed and get routed to **both**
      versions over several tries (not just one)
